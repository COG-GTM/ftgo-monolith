...
// Path variables are bound by explicit name: Spring 6 no longer recovers parameter names from the
// bytecode debug info, so an unnamed @PathVariable only works when compiled with -parameters.
@RestController
public class CourierController {
  ...
  public ResponseEntity<String> updateCourierLocation(@PathVariable("courierId") long courierId, @RequestBody CourierAvailability availability) {
  ...
  public ResponseEntity<Courier> get(@PathVariable("courierId") long courierId) {
  ...
  public ResponseEntity<String> updateLocation(@PathVariable("courierId") long courierId, @RequestBody CourierLocationUpdate locationUpdate) {
  ...
  public ResponseEntity<CourierWorkloadResponse> getWorkload(@PathVariable("courierId") long courierId) {
    Courier courier = courierService.findCourierById(courierId);
